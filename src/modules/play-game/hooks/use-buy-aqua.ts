...
export const useBuyAquaTelegram = (props: IPropsUseBuyAqua) => {
  const { addNotification } = useNotification();
  const [tonConnectUI, setOptions] = useTonConnectUI();
  const [isLoadingTelegram, setIsLoading] = useState(false);
  const { transaction, onBuySuccess } = props;
  const { setInterval } = useAccountInfoContext();
  const handleBuyAquaTelegram = useCallback(async () => {
    try {
      setIsLoading(true);
      console.log('transaction', transaction);
      const res = await tonConnectUI.sendTransaction(transaction);
